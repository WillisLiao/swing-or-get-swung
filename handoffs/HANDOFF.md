...
**Latest - in-game personal K/D combat information:** the gameplay HUD now shows the local player's current-match `KILLS` and `DEATHS` in a compact top-left safe-area plate. `RiftlineMatch` owns the authoritative per-actor totals: valid opposing-team eliminations award a kill, every accepted defeat awards a death, respawning preserves the totals, and starting a rematch clears them. The same data rides inside the existing LAN match snapshot and replica clients sanitize/apply it, so the HUD is not client-authored and reconnects keep the host's count. HUD redraw is change-guarded. Headless import and all 18 exercises pass; MCP live evaluation proved an authoritative host kill immediately updated the local HUD, the 1280x588 visual was inspected with double-digit values, and runtime logs had no new project errors (only the existing benign physics-interpolation camera warning). No phone or iPad was launched or tested for this change, per the user's explicit request.
...
Headless import is clean and all 18 exercises pass.
...
- Godot binary on Robert's Mac: `/Users/robertwu/Desktop/Godot.app/Contents/MacOS/Godot` (Godot 4.7.1). Always export `GODOT_BIN=/Users/robertwu/Desktop/Godot.app/Contents/MacOS/Godot` on this machine. The older `/opt/homebrew/bin/godot` and the stale handoff path under `Downloads` no longer exist.
