...
**Latest - sniper anatomical one-shot zones, on `codex/sniper-hit-zones`, not yet merged:** authoritative projectile impacts now classify a Duelist's stance-scaled capsule as `head`, central upper `torso`, or `limb` (arms, legs, and lower abdomen). The Longview Mk1 sniper raises head and torso damage to 100 so either eliminates a full-health unshielded target in one shot; limb/lower-body hits retain the existing 90-near to 70-far falloff and therefore do not one-shot full health. Every other weapon and the ballistic shield's frontal block are unchanged. The classification runs only after a projectile already impacts a hostile Duelist, adds no collision nodes and no frame/physics-tick work. The branch was rebased onto upstream Concourse V2 commit `e471330`; clean headless import and all 19 exercises pass. Godot MCP 3.1.2 launched the live main scene, returned `head/torso/limb/limb` for the four anatomical probes, and reported no runtime errors. The signed build was installed and launched on Robert's paired iPhone 16 Pro Max and process-verified; no iPad was launched. Human aiming/feel confirmation on the phone remains required.
