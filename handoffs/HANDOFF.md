...
**Latest, filed for a follow-up session - seven visual/logic bugs from the deployed iPad build plus a shader/lighting issue (`handoffs/NEXT-SESSION-weapon-visual-bugs.md`, not yet fixed):**
The user played the deployed build after the ninth session below and found six real problems (ADS reticle not moving with recoil, recoil/muzzle-flash intermittently dead after respawn/class-switch, the sniper showing no recoil while scoped, the sniper's reticle should be a dot not a cross, pistol hands wrong - should be one-handed at hip and the support hand comes from the wrong side entirely, and the shield's default/ADS composition still wrong), then immediately after the `IOSapp` migration below, a seventh: a moving specular "stripe" artifact on every `nuclear_pbr`-shaded surface (weapons, hands, arena cover), root-caused to the shader's sine-wave normal-perturbation technique reading as periodic specular banding under real sky reflections - not yet fixed, diagnosis and a first fix direction are in the bootstrap file.
...
- Four classes (frontline/sniper/runner/shield) resolve into 1-2 loadout slots via `Duelist.configure_loadout()`; `RiftlineRoster` carries `player_class`/`primary_weapon` (defaults to Frontline/Rifle - no selection screen yet, see below). Runner still wears the nuclear vest as visual class equipment, but carrying the core no longer damages any class; shield blocks frontal damage.
...
**Same day, eighth session - core carry health drain removed:**
- Per the user's direct playtest decision, carrying the nuclear core never deals damage, regardless of class or vest. The old 2.5 HP/s non-Runner drain and its constant are removed.
...
- `handoffs/NEXT-SESSION-weapon-visual-bugs.md` - **highest priority**, filed straight from the deployed iPad build plus a live post-migration check. Seven bugs: ADS reticle not moving with recoil, recoil/muzzle-flash intermittently dead after respawn/class-switch, sniper showing no recoil while scoped, sniper reticle should be a dot not a cross, pistol hands wrong (one-handed at hip only, support hand from the wrong side), the shield's default/ADS composition still wrong, and a moving specular "stripe" artifact on every `nuclear_pbr`-shaded surface (root-caused to the shader's sine-wave normal perturbation, not yet fixed). Also touches `shaders/nuclear_pbr.gdshader` in addition to `scripts/duelist.gd`/`scripts/duel_hud.gd` below.
