...
func _initialize() -> void:
	...
	assert(int(host.team) == int(Duelist.Team.SUN))
	var first_remote := app_roster.assign_peer(7)
	assert(str(first_remote.actor_id) == "peer_7")
	assert(int(first_remote.team) == int(Duelist.Team.VOID))
	assert(app_roster.is_ready())

	var copy := app_roster.record("host")
	copy.team = int(Duelist.Team.VOID)
	assert(int(app_roster.record("host").team) == int(Duelist.Team.SUN))
	...
	assert(_count_team(records, Duelist.Team.SUN) == 5)
	assert(_count_team(records, Duelist.Team.VOID) == 5)
	...
	assert(int(tie_remote.team) == int(Duelist.Team.SUN))
